...
var surveyDataTabulator = Tabulator()

// Loop through surveyData, incrementing the count for each response.
for data in surveyData {
    surveyDataTabulator.incrementCount(forValue: data)
}

// Loop through the tallied shows (stored in tabulator.values), printing the information from each one.
for value in surveyDataTabulator.values {
    print("Count of \"\(value)\" is \(surveyDataTabulator.count(forValue: value))")
}
/*:
...
var surveyValidDataTabulator = Tabulator()

// Create a variable to keep a count of the errors.
var errorDataTabulator = Tabulator()


// Loop through all tabulator values. Count only those that contained in showCatalog. Else count errors. Print the count for those that are contained in showCatalog.
for value in surveyDataTabulator.values {
    if showCatalog.contains(value) {
        surveyValidDataTabulator.incrementCount(forValue: value)
    } else {
        errorDataTabulator.incrementCount(forValue: value)
    }
}

print(surveyValidDataTabulator.values)

print("\n\n***** DATA ERRORS ******\n\n")

// Print the error count.
print(errorDataTabulator.values)
/*:
