...
var randomShowTabulator = Tabulator()

// Loop through the shows in randomShowData, incrementing the count for each one.
for randomShow in randomShowDataLargeSet {
    randomShowTabulator.incrementCount(forValue: randomShow)
}
print("Array of shows \(randomShowTabulator.values)")

// Loop through the tallied shows (stored in tabulator.values), printing the information from each one.
for value in randomShowTabulator.values {
    print("Count of \(value) is \(randomShowTabulator.count(forValue: value))")
}
