...
func closestMatch(for string: String, from potentialMatches: [String]) -> String {
    ...
    for i in 0 ..< potentialMatches.count {
        ...
        if editDistance(from: string, to: potentialMatches[i]) < bestEditDistance {
            bestEditDistance = editDistance(from: string, to: potentialMatches[i])
            bestMatchIndex = i
        }
...
lowercaseCatalog
closestMatch(for: "begond the night", from: lowercaseCatalog)
...
var surveyDataTabulatorLowercased = Tabulator()

// Loop through surveyData. Make a lowercase version of each value.
for data in surveyData {
    surveyDataTabulatorLowercased.incrementCount(forValue: data.lowercased())
}
// Create a variable to keep a count of valid data.
var surveyValidDataTabulatorLowercased = Tabulator()

// Create a variable to find the closest match.
var closestData = ""
...
for value in surveyDataTabulatorLowercased.values {
    if lowercaseCatalog.contains(value) {
        surveyValidDataTabulatorLowercased.incrementCount(forValue: value)
    } else {
        closestData = closestMatch(for: value, from: lowercaseCatalog)
        surveyValidDataTabulatorLowercased.incrementCount(forValue: closestData)
    }
}
// Loop through all tabulator values. Print only those that are contained in the lowercase version of the show catalog.
for value in surveyValidDataTabulatorLowercased.values {
    print("Count of \"\(value)\" is \(surveyValidDataTabulatorLowercased.count(forValue: value))")
}

print("\n\n***** DATA ERRORS ******\n\n")

// Create a variable to keep a count of the errors.
var errorCount = Tabulator()
...
for value in surveyValidDataTabulatorLowercased.values {
    if lowercaseCatalog.contains(value) {
        ...
        errorCount.incrementCount(forValue: value)
    }
}

// Print the error count.
print("Number of errors — \(errorCount.values)")
/*:
